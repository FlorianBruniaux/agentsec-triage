...
paths. The runner validated each plan and produced these plan digests. It did
not create approval receipts:

| Project | Exact image ID | Runtime arguments | Plan digest |
| --- | --- | --- | --- |
...
These digests authorize nothing by themselves. Each `execute` call requires the
matching digest and a separate procedural approval receipt to be supplied
explicitly. The receipt records a declared identity but does not authenticate
that identity.
...
- the runtime plan digest generated by
  `scripts/run_competitive_benchmark.py validate`; the command does not create
  a receipt.

Only a separate procedural receipt for each clean-control digest can start
`docker run` against a fixture.
